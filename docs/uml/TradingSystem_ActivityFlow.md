...
**Déclencheur :** Le **System Core** sort du mode veille suite à un signal temporelle programmé par le **Market Clock** (ex: 8h00 AM).

**Orchestration :** Le **System Core**  :
...
Cette étape lance des processus en parallèle pour garantir que la prise de décision puisse ẽtre immédiate à l'ouverture :

* **Trading/Risk Core :** Le **Trading/Risk Core** charge en mémoire les paramètres essentiels à l'exécution :
    * **Jour de Rebalancement :** Charge les ordres de rebalancement créés lors de la phase post-marché de la dernière session de trading et stockés en base (`Portfolio State Manager`)
    * **Jour de Trading Normal :** Charge les données de **stop-loss** et de take-profit relatives aux positions en cours (`Risk Monitor`).
      
* **Data Core :** Le **Data Core** démarre l'écoute et l'acquisition des données de marché :
...
* **Déclenchement :** Le **System Core** bascule en phase **In-Trade** uniquement après avoir reçu un nouveau signal du **Market Clock** indiquant que l'heure d'ouverture est atteinte.
...
* **Déclencheur :** Le **System Core** reçoit le signal de fermeture du **Market Clock**.
...
* **Exécution de la Stratégie (Trading/Risk Core) :**
    ...
    * **Objectif :** Ce moteur calcule les nouvelles demandes d'ordres cibles et les enregistre en base de données pour être chargées le lendemain matin.
* **Transition :** Une fois toutes les tâches Post-Trade complétées et validées, le **System Core** bascule en phase **Off-Cycle** (Veille).
