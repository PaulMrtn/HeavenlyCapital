...
Ce processus décrit les étapes critiques qui suivent la réception d'une confirmation d'exécution d'ordre (`Fill`). Il garantit la mise à jour atomique et immédiate de l'état financier critique via le **Portfolio Manager (PM)**, et sépare la journalisation d'audit dans un flux asynchrone à basse priorité.
...
| **C2-C3** | Order Manager $\rightarrow$ PM | Transfert du contrôle : Le PM calcule le PnL réalisé et crée les Lots (Acquisition/Realization) **en mémoire**. |
| **C-FORK** | PM | **Nœud de Fourche :** L'état en mémoire est à jour. Le PM initie deux chemins **concurrents** pour la persistance et l'audit. |
...
| :--- | :--- | :--- |
| **C4** | **PM** $\rightarrow$ DIL | **Soumission de la Tâche de Persistance Critique :** Le PM initie l'écriture immédiate de l'état critique (Lots/Positions) au DIL. |
| **C5** | DIL | **Écriture Atomique :** Le DIL gère la transaction ACID vers la base de données. |
| **C6** | PM | **Fin du Processus Critique** (Le PM peut être libéré). |
...
| Étape | Composant | Action après le Nœud de Fourche |
| :--- | :--- | :--- |
| **L0** | **PM** $\rightarrow$ Log Service | **Notifier le Log Service (Asynchrone) :** Le PM notifie le Log Service de lancer le processus d'audit. |
