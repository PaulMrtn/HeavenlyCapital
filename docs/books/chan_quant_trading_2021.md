...
- L’idée consiste à entraîner un modèle de machine learning en utilisant comme inputs les paramètres de la stratégie de trading ainsi que des indicateurs techniques (Bollinger, MACD, RSI, etc.). L’output est le rendement obtenu avec cette configuration. Une fois le modèle entraîné, on effectue un grid search uniquement sur les paramètres de la stratégie : le modèle de ML prédit le rendement associé à chaque combinaison.
